...
const ruleName = 'openstax/no-unit'
...
const endsWithUnit = (value) => {
  for (const unit of units) {
    const previousChar = value.length > unit.length ? value.charAt(value.length - unit.length - 1) : ''
    if (value.endsWith(unit) && (previousChar === '}' || /\d/.test(previousChar))) {
      return true
    }
  }
  return false
...
const hasForbiddenPX = (node, options) => {
  ...
  parsed.walk(node => {
    ...
    if (node.type === 'word' && (matched = node.value.match(/^([-,+]?\d+(\.\d+)?)px$/))) {
      ...
      // eg. ~'@{width}px'
      hasPX = true
    } else if ((node.type === 'string' || node.type === 'word') && endsWithUnit(node.value)) {
      // Check the rest of the units
      hasPX = true
    }
